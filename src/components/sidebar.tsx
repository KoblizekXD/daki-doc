    ...
    <div className='py-2 relative cursor-pointer m-2 rounded-md flex items-center px-4 hover:bg-muted transition-colors'>
      ...
      {version instanceof Array && (
        <>
          ...
          <div
            className={`absolute max-h-[500%] overflow-y-scroll ${!opened && 'hidden'} flex flex-col rounded border bg-background p-0.5 w-inherit left-0 top-[110%] right-0`}>
            {version instanceof Array &&
              version.map((v, i) => (
                <div
                  onClick={() => {
  ...
  return (
    <div className='flex overflow-hidden flex-col mx-2 my-1'>
      <div
        ...
        <Image width={20} height={20} alt='xd' src={icon} />
        {title}
        {children &&
          (opened ? (
  ...
  return (
    <Sidebar>
      ...
      {Object.entries(tree).map((group, i) => (
        <SidebarItem
          defaultState={group[1].some(
            (item: any) => selectedClass === item.type,
          )}
          key={i}
          title={group[0]}
          icon={PACKAGE_ICON}>
          {group[1].map((item: any, j: number) => (
            <SidebarItem
              ...
              onSelected={() => {
                const split = artifact.split(':');
                router.push(
                  `/javadocs/${split[0]}:${split[1]}/${selected}/${item.type}`,
                );
              }}
            />
          ))}
